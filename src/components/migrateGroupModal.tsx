...
const Main = observer((props: IProps) => {
  ...
  const submit = async () => {
    ...
    await sleep(300);
    try {
      const group = await SeedApi.migrate(state.seedUrl, props.oldGroupId);
      state.group = group;
  ...
  return (
    <div className="bg-white dark:bg-[#181818] py-8 px-10 w-[350px] box-border">
      <div className="text-18 font-bold dark:text-white dark:text-opacity-80 text-gray-700 text-center">添加 V2 版本的种子网络</div>
